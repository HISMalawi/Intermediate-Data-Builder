...
def populate_hiv_staging_info
  ...
  SELECT patient_id FROM #{@rds_db}.patient WHERE updated_at >= '#{last_updated}';
SQL
  ...
  patients.each do |patient|
    ...
    if staging_exist
      ...
      staging_info.transfer_in = get_transfer_in(patient['patient_id'], get_date_enrolled(patient['patient_id']))
      staging_info.re_initiated = get_transfer_in(patient['patient_id'], get_date_enrolled(patient['patient_id']))
      staging_info.age_at_initiation = patient_age_at_initiation(patient['patient_id'])['age_at_initiation'] rescue nil
      staging_info.age_in_days_at_initiation = patient_age_at_initiation(patient['patient_id'])['age_in_days'] rescue nil
...
def get_who_stage(patient_id)
  ...
  SELECT value_coded, value_coded_name_id FROM #{@rds_db}.obs WHERE concept_id = 7562 AND person_id = #{patient_id};
SQL
begin
  who_stage = ActiveRecord::Base.connection.select_one <<SQL
  SELECT name FROM #{@rds_db}.concept_name WHERE concept_id = #{concept_answer_id['value_coded']} 
...
SQL
  ...
  return  who_stage['name'] rescue nil
end
...
begin
  ...
  SELECT name FROM #{@rds_db}.concept_name WHERE concept_id = #{coded_concept_id['value_coded']} AND LENGTH(name) > 0;
SQL
rescue
  nil
end

return reason_for_art_eligibility['name'] rescue nil
