...
def populate_hiv_staging_info
  ...
  staging = ActiveRecord::Base.connection.select_all <<~SQL
...
Parallel.each(staging, progress: 'Processing Staging') do |patient|
  ...
  update_last_update('StagingInfo', staging.last['updated_at'])
end
